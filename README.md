...
Prebuilt virtual machine QCOW2 images for x86_64, AArch64, LoongArch64 and RISC-V can be obtained from:
...
(qemu) help
(qemu) screendump /path/to/screenshot.ppm
...
The systemd service opens the associated `/dev/tapX` device and passes it to QEMU as file descriptor 3. The matching VM configuration entry:
```ini
# test-macvtap.conf
